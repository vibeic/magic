    ...
    const char *inName)
{
    ...
    /* Read the version through the MagicVersion extern (utils/magic_buildinfo.h,
     * pulled in via utils/magic.h) rather than the raw -DMAGIC_VERSION define.
     * Upstream isolated those defines into the single utils/buildinfo.c
     * translation unit, so MAGIC_VERSION is no longer on this file's compile
     * command line.  MagicVersion is initialized from MAGIC_VERSION, so the
     * string emitted here is unchanged.
     */
    fprintf(f, "*VERSION \"%s\"\n", MagicVersion);
